(ns kernel_neuro.main-test
  "Contract + behavioral test for the kernel_neuro-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "kernel_neuro-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'kernel_neuro.main-test)]
