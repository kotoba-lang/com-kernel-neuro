(ns kernel_neuro.main
  "Kotodama WASM entrypoint for the kernel_neuro clean-room actor (L4) — Clojure port.
  ...
  facts (`kernel_neuro.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "kernel_neuro")
(def tier "L4")
...
(def entity-specs
  [{:entity "Device"      :plural "devices"      :id-prefix "kernelne_dev"
    :fields [:model :channels :samplingHz :firmware] :required [:model :channels]
    :coerce {:channels :int :samplingHz :int}        :refs {}}
   {:entity "Session"     :plural "sessions"     :id-prefix "kernelne_ses"
...
(defn emit-facts
  "EAVT facts for one record: {\"kernel_neuro.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kernel_neuro-compat" :tier tier :entities entities} 200])
...
(defn start! [] :kernel_neuro-compat/ready)
